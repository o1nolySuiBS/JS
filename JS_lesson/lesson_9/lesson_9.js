...
for (const user of coursesAndDurationArray) {
    let div = document.createElement('div')
    div.innerHTML = `<hr>title - ${user.title} <br/> MonthDuration - ${user.monthDuration}<hr>`;
    document.body.appendChild(div)
}
// =========================
...
for (const Element of coursesAndDurationArray) {
    ...
    h1.innerHTML = `<hr> title - ${Element.title} <hr>`

    let p = document.createElement('p')
    p.classList.add('description')
    p.innerHTML = ` MonthDuration - ${Element.monthDuration}<br>`
...
simpsons.forEach(person => {
    ...
    imgg.src = person.photo;
    imgg.alt = '';

    let pp = document.createElement('p');
    pp.innerHTML = `${person.info}`;

    divk.append(h2, imgg, pp);
